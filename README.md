...
```
**Note**: During the init process, kratix will convert the Helm values k=v file content into a `openAPIV3Schema` and assign it as the custom resource API definition using also the group and kind provided
...
...
```
Next, create the needed folders/files
```shell
mkdir -p kratix/{input,output}; touch kratix/{input,output}/object.yaml
```
Copy the promise request example to the kratix/input/object.yaml. **Note**: The name of the Custom Resource spec will be used as helm chart name 
```shell
cp example-resource.yaml kratix/input/object.yaml
```
To generate the helm chart, you can run either this script executing a `helm template` command from the url of the chart:
```shell
...
```
or to use the kratix `helm-resource-configure` container `ghcr.io/syntasso/kratix-cli/helm-resource-configure:v0.1.0`
```shell
podman run --rm \
